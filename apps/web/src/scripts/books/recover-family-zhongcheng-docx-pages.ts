...
const BOOK_TITLE = "家庭常见病中成药使用指南";
const DEFAULT_DOCX_PATH = "C:\\Users\\Sagistariam\\Downloads\\222.docx";
const WORD_TO_BOOK_PAGES = [2, 9, 35, 38, 75, 88, 103, 157, 161, 183, 184, 185];
...
function resolveDocxPath() {
  return process.env.BOOK_FAMILY_ZHONGCHENG_SKIPPED_DOCX_PATH?.trim() || DEFAULT_DOCX_PATH;
}
...
async function extractWordPages(docxPath: string) {
  ...
  const outPath = path.join(artifactRoot, "family-zhongcheng-docx-pages-extracted.json");
...
async function findBookDocument(sql: Sql) {
  ...
  if (!rows[0]) {
    throw new Error("Family zhongcheng book document not found in source_documents.");
  }
  ...
  for (const chunk of chunks) {
    await input.sql`
      insert into public.source_chunks (
        ...
        ${sectionRows[0].id},
        ${input.sourceId},
        ${400000 + input.bookPage * 100 + chunk.chunkIndex},
        ${chunk.originalText},
        ${chunk.chunkHash},
        ${sectionKey},
        ${sectionTitle},
...
async function main() {
  ...
  if (!existsSync(docxPath)) {
    throw new Error("User-provided family zhongcheng skipped-page DOCX was not found.");
  }
  ...
  await writeJsonFile(
    path.join(getBookArtifactRoot(), "family-zhongcheng-skipped-docx-recovery.json"),
    report,
  );

  console.log("Family zhongcheng skipped-page DOCX recovery complete. Page text was not printed.");
  console.log(
    JSON.stringify({
...
main().catch((error) => {
  console.error(
    error instanceof Error
      ? error.message
      : "Family zhongcheng skipped-page DOCX recovery failed safely.",
  );
  process.exit(1);
});
